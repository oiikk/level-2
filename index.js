...
const LEVEL_ROLES = {
  ...
  25: "1463098038376730644",
  30: "1480859771363135520",
  35: "1480859921057841243",
  40: "1480859992448958568",
  45: "1480860068000960635",
  50: "1480860143573925960",
};
...
.then(() => {
  ...
  importLevels(); // استيراد البيانات عند الاتصال
})
...
async function importLevels() {
  if (!fs.existsSync('./levels_backup.json')) return;
  const data = JSON.parse(fs.readFileSync('./levels_backup.json', 'utf8'));
  for (const user of data) {
    await Level.updateOne(
      { userId: user.userId },
      { $set: { xp: user.xp, level: user.level, lastMessage: user.lastMessage } },
      { upsert: true }
    );
  }
  console.log("✅ تم استيراد كل المستويات إلى MongoDB الجديد!");
}
...
client.on("messageCreate", async (message) => {
  ...
  if (now - xpCooldowns[userId] >= XP_COOLDOWN) {
    ...
    if (!data) data = await Level.create({ userId, xp: 0, level: 0, lastMessage: 0 });
  ...
  if (command === "top") {
    ...
    const titleImageURL = "https://media.discordapp.net/attachments/1473286639840268431/1480426834423316635/e468943da8c63857.png"; 
    try {
